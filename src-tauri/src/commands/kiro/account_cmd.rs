...
#[tauri::command]
pub async fn import_account_from_external(
    state: State<'_, AppState>,
    request: ImportAccountRequest,
) -> Result<Account, String> {
    println!("[import_account_from_external] 接收到外部账号导入请求");
    println!("Email: {}", request.email);
    println!("Provider: {:?}", request.provider);
    
    let region = request.region.unwrap_or_else(|| "us-east-1".to_string());
    let provider = request.provider.unwrap_or_else(|| "BuilderId".to_string());
    let label = request.label.unwrap_or_else(|| format!("Kiro {} 账号", provider));
    ...
    let machine_id = get_machine_id();
    let cw_client = CodeWhispererClient::new(&machine_id);
    let usage_call = cw_client.get_usage_limits(&request.access_token).await;
    let (usage, is_banned) = match &usage_call {
        Ok(u) => (Some(u.clone()), false),
        Err(e) if e.starts_with("BANNED:") => (None, true),
        Err(_) => (None, false),
    ...
    // 计算过期时间（假设3600秒）
    let expires_at = chrono::Local::now() + chrono::Duration::seconds(3600);
    
    let mut store = state.store.lock().unwrap();
    
    // 按 email + provider 去重
    let account = if let Some(existing) = store.accounts.iter_mut().find(|a| 
        a.email == request.email && a.provider.as_deref() == Some(&provider)
    ) {
        println!("[import_account_from_external] 更新现有账号");
        existing.access_token = Some(request.access_token);
        existing.refresh_token = Some(request.refresh_token);
        existing.user_id = user_id;
        existing.expires_at = Some(expires_at.format("%Y/%m/%d %H:%M:%S").to_string());
        existing.client_id = Some(request.client_id);
        existing.client_secret = Some(request.client_secret);
        existing.region = Some(region);
        existing.client_id_hash = Some(client_id_hash);
        existing.id_token = request.id_token;
        existing.usage_data = Some(usage_data);
        existing.status = if is_banned { "已封禁".to_string() } else { "正常".to_string() };
        existing.label = label;
        existing.clone()
    } else {
        println!("[import_account_from_external] 创建新账号");
        let mut account = Account::new(request.email.clone(), label);
        ...
        account.user_id = user_id;
        account.expires_at = Some(expires_at.format("%Y/%m/%d %H:%M:%S").to_string());
        account.client_id = Some(request.client_id);
        account.client_secret = Some(request.client_secret);
        account.region = Some(region);
        account.client_id_hash = Some(client_id_hash);
        account.id_token = request.id_token;
        account.usage_data = Some(usage_data);
        account.status = if is_banned { "已封禁".to_string() } else { "正常".to_string() };
        store.accounts.insert(0, account.clone());
        account
    ...
    store.save_to_file();
    println!("[import_account_from_external] 账号导入成功: {}", account.id);
